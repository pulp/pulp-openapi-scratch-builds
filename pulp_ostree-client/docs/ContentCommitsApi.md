...
# **read**
> OstreeOstreeCommitResponse read(ostree_ostree_commit_href, fields=fields, exclude_fields=exclude_fields)
...
with pulpcore.client.pulp_ostree.ApiClient(configuration) as api_client:
    ...
    try:
        # Inspect an ostree commit
        api_response = api_instance.read(ostree_ostree_commit_href, fields=fields, exclude_fields=exclude_fields)
        print("The response of ContentCommitsApi->read:\n")
        pprint(api_response)
    except Exception as e:
        print("Exception when calling ContentCommitsApi->read: %s\n" % e)
```
